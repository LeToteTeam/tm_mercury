defmodule TM.Mercury.Protocol.Command do
  ...
  def build(%Reader{} = rdr, [op_name | args]) do
    with {:ok, code} <- Opcode.encode(op_name), do: build_command(rdr, {op_name, code}, args)
  end
  ...
  defp build_command(rdr, {:set_region, _code} = op, [region]) when is_atom(region) do
    with {:ok, region_code} <- Region.encode(region), do: build_command(rdr, op, [region_code])
  end
  ...
  defp build_command(_rdr, {:set_antenna_port, code}, [ports]) when is_list(ports) do
    ant =
      Enum.reduce(ports, <<>>, fn {tx, rx}, ant ->
        ant <> <<tx, rx>>
      end)
  ...
  defp encode_param(_rdr, :antenna_control_gpio, value), do: {:ok, <<value>>}

  defp encode_param(_rdr, :trigger_read_gpio, value), do: {:ok, <<value>>}

  defp encode_param(_rdr, :unique_by_antenna, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :unique_by_data, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :unique_by_protocol, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :extended_epc, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :safety_antenna_check, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :safety_temperature_check, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :record_highest_rssi, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :rssi_in_dbm, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :self_jammer_cancellation, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :enable_read_filter, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :send_crc, value), do: {:ok, <<to_integer(value)>>}

  defp encode_param(_rdr, :read_filter_timeout, value), do: {:ok, <<value::32-signed>>}
  ...
  defp encode_param(_rdr, key, _value), do: {:error, {:param_key_not_found, key}}
end
